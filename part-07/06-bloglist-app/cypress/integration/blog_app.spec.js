describe('Blog app', () => {
  ...
  describe('When two users exists and one of them is logged in', () => {
    beforeEach(() => {
      cy.login({ username: 'lmaero.pro', password: 'topSecret' });
      cy.createBlog({
        author: 'Luis Guzman',
    ...
    it('A blog cannot be deleted if its not authored by logged in user', () => {
      cy.get('#log-out-button').should('be.visible');
    ...
    it('A blog can be deleted if its authored by logged in user', () => {
  ...
  describe('When there are several blogs', () => {
    ...
    it.only('Blogs appear ordered by number of likes', () => {
      cy.get('#log-out-button').should('be.visible');
